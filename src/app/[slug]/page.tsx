...
export default async function AgencyPage({
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <header className="w-full bg-white dark:bg-slate-900 border-b dark:border-slate-800 sticky top-0 z-50 shadow-xs transition-colors">
        <div className="max-w-4xl mx-auto px-4 h-14 md:h-16 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            {agency.logoImageUrl ? (
              ...
              <div className="h-8 w-8 md:h-9 md:w-9 rounded-lg bg-slate-900 dark:bg-slate-800 text-white font-bold flex items-center justify-center text-sm shrink-0">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-5 md:py-8 space-y-5 md:space-y-6">
        ...
        <section className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-4 md:p-6 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <div>
              <span className="text-[10px] md:text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider bg-emerald-50 dark:bg-emerald-950/50 px-2 py-0.5 rounded">
            ...
            <div className="w-full sm:w-1/2 space-y-3.5">
              <div className="flex flex-wrap gap-2 text-xs font-semibold">
                <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-md flex items-center gap-1">
                  <Users className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" /> {car.seats} Seats
                </span>
                <span className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-2.5 py-1 rounded-md flex items-center gap-1">
                  <Car className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" /> {car.hasAc ? "AC Cab" : "Non-AC"}
                </span>
                <span className="bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-400 px-2.5 py-1 rounded-md flex items-center gap-1">
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" /> Commercial Driver
                </span>
              </div>
        ...
        <section className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-xs p-4 md:p-6 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-base md:text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-1.5">
              <Info className="h-4 w-4 text-emerald-600 dark:text-emerald-400" /> About {agency.name}
            </h3>
            {agency.ownerName && (
              <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                Owner: <strong className="text-slate-800 dark:text-slate-200">{agency.ownerName}</strong>
              </span>
            )}
          </div>
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {otherCars.map(({ car: altCar, journeyPrice: altPrice }) => (
                <div 
                  key={altCar.id}
                  className="bg-white dark:bg-slate-900 border dark:border-slate-800 rounded-xl p-3.5 shadow-xs flex items-center justify-between gap-3"
                >
                  <div className="flex gap-3 items-center min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-slate-500 dark:text-slate-400">{altCar.seats} Seats • {altCar.hasAc ? "AC" : "Non-AC"}</p>
      ...
      {/* Minimal Footer */}
      <footer className="w-full border-t dark:border-slate-800 bg-white dark:bg-slate-900 py-4 mt-auto">
        <div className="max-w-4xl mx-auto px-4 text-center text-xs text-slate-500 dark:text-slate-400">
          Powered by <span className="font-bold text-slate-800 dark:text-slate-200">SaffarSathi</span>
        </div>
      </footer>
